...
- that consistency report now also carries `decisionSource`, `expectedSummary`, `actualSummary`, `deviationSummary`, and a compact `causeLine`, so fallback reasons are visible without opening raw snapshots
...
- those reports now expose both the chosen source (`decisionSource`) and one compact `causeLine` with expected value, actual value, detected deviation, and fallback source
