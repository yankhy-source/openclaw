...
- `pnpm qa:local-agents:ops-status` prints the current operating state across selftest, intelligence loop, recovery, human WhatsApp, and the deeper human WhatsApp conversation path, with warning-only recent-history counters.
- `pnpm qa:local-agents:trend` reads recent per-run summaries for intelligence, recovery, human WhatsApp, and the deeper human WhatsApp conversation path and exits non-zero on visible regressions in the inspected window.
...
- `pnpm qa:local-agents:intelligence-loop` runs the full human loop: human eval, WhatsApp human eval, the deeper WhatsApp conversation eval, and a separate evaluator review with a next upgrade recommendation stored under `.local-agent-intelligence-loop/` plus `.local-agent-last-intelligence-loop.json`.
