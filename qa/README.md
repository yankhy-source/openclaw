...
- `pnpm qa:local-agents:context-report-audit` bundles the current green-path and forced-fallback WhatsApp context reports into one audit artifact, and fails if any report is missing the fixed `erwartet/tatsaechlich/abweichung/quelle` template. Outputs live under `.local-agent-context-report-audit/` plus `.local-agent-last-context-report-audit.json`.
