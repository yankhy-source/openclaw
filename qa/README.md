...
- the WhatsApp human, conversation, resume, and resume-failure evals now also persist step-level consistency reports that record whether token/session/marker checks passed cleanly or why a fallback path was required, including `decisionSource`, `expectedSummary`, `actualSummary`, `deviationSummary`, and a compact `causeLine`.
