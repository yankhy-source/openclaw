...
PROBE_SUMMARY_PATH="${OPENCLAW_QWEN_SESSIONS_PROBE_SUMMARY_PATH:-$REPO_ROOT/.local-agent-last-qwen-sessions-probe.json}"
PROBE_ROOT="$(mktemp -d "$REPO_ROOT/.local-qwen-sessions-probe.XXXXXX")"
...
cleanup() {
  if [[ "$PROBE_EXIT_CODE" -eq 0 ]]; then
    ...
    echo "preserving qwen sessions probe artifacts: $PROBE_ROOT" >&2
  fi
}
...
if payload is not None:
    ...
    lower = raw.lower()
    if "oauth token refresh failed for qwen-portal" in lower:
        reason = "qwen_auth_refresh_failed"
...
PY
  ...
  if [[ -z "$PROBE_REASON" && -f "$OUTPUT_LOG" ]]; then
    if rg -q 'OAuth token refresh failed for qwen-portal' "$OUTPUT_LOG"; then
      PROBE_REASON="qwen_auth_refresh_failed"
    elif rg -q 'did not expose the sessions_spawn runtime tool' "$OUTPUT_LOG"; then
...
on_exit() {
  ...
  if [[ "$PROBE_EXIT_CODE" -ne 0 && -z "$PROBE_FAILED_COMMAND" ]]; then
    PROBE_FAILED_COMMAND="qwen sessions probe exited with code $PROBE_EXIT_CODE"
  fi
  write_probe_summary
  cleanup
...
set +e
OPENCLAW_SELFTEST_MANAGER_ID=oc-selftest-qwen \
OPENCLAW_MAIN_ORCHESTRATOR_CHILD_AGENT_ID=oc-builder-qwen \
OPENCLAW_MAIN_ORCHESTRATOR_EXPECT_CHILD_PROVIDER=qwen-portal \
OPENCLAW_MAIN_ORCHESTRATOR_EXPECT_CHILD_MODEL=coder-model \
