...
PROBE_SUMMARY_PATH="${OPENCLAW_GEMINI_SESSIONS_PROBE_SUMMARY_PATH:-$REPO_ROOT/.local-agent-last-gemini-sessions-probe.json}"
PROBE_ROOT="$(mktemp -d "$REPO_ROOT/.local-gemini-sessions-probe.XXXXXX")"
...
cleanup() {
  if [[ "$PROBE_EXIT_CODE" -eq 0 ]]; then
    ...
    echo "preserving gemini sessions probe artifacts: $PROBE_ROOT" >&2
  fi
}
...
on_exit() {
  ...
  if [[ "$PROBE_EXIT_CODE" -ne 0 && -z "$PROBE_FAILED_COMMAND" ]]; then
    PROBE_FAILED_COMMAND="gemini sessions probe exited with code $PROBE_EXIT_CODE"
  fi
  write_probe_summary
  cleanup
...
set +e
OPENCLAW_SELFTEST_MANAGER_ID=oc-selftest-gemini \
OPENCLAW_MAIN_ORCHESTRATOR_CHILD_AGENT_ID=oc-builder-gemini \
OPENCLAW_MAIN_ORCHESTRATOR_EXPECT_CHILD_PROVIDER=google-gemini \
OPENCLAW_MAIN_ORCHESTRATOR_EXPECT_CHILD_MODEL=gemini-2.0-flash \
