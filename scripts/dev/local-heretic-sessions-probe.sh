...
PROBE_SUMMARY_PATH="${OPENCLAW_HERETIC_SESSIONS_PROBE_SUMMARY_PATH:-$REPO_ROOT/.local-agent-last-heretic-sessions-probe.json}"
PROBE_ROOT="$(mktemp -d "$REPO_ROOT/.local-heretic-sessions-probe.XXXXXX")"
...
cleanup() {
  if [[ "$PROBE_EXIT_CODE" -eq 0 ]]; then
    ...
    echo "preserving heretic sessions probe artifacts: $PROBE_ROOT" >&2
  fi
}
...
on_exit() {
  ...
  if [[ "$PROBE_EXIT_CODE" -ne 0 && -z "$PROBE_FAILED_COMMAND" ]]; then
    PROBE_FAILED_COMMAND="heretic sessions probe exited with code $PROBE_EXIT_CODE"
  fi
  write_probe_summary
  cleanup
...
set +e
OPENCLAW_SELFTEST_MANAGER_ID=oc-selftest-heretic \
OPENCLAW_MAIN_ORCHESTRATOR_CHILD_AGENT_ID=oc-builder-heretic \
OPENCLAW_MAIN_ORCHESTRATOR_EXPECT_CHILD_PROVIDER=heretic-local \
OPENCLAW_MAIN_ORCHESTRATOR_EXPECT_CHILD_MODEL=qwen3-4b-instruct-2507 \
