...
cleanup() {
  if [[ -n "$SUMMARY_WORKSPACE_PATH" && -f "$SUMMARY_WORKSPACE_PATH" ]]; then
    rm -f "$SUMMARY_WORKSPACE_PATH"
  fi
}
trap cleanup EXIT
...
if ! bash "$DOCTOR_SCRIPT" --require-mode live --max-age-seconds "$MAX_AGE_SECONDS" >/dev/null 2>&1; then
  echo "== refresh live baseline =="
...
SUMMARY_WORKSPACE_PATH="$(workspace_mirror_file "$SUMMARY_PATH" "whatsapp-client-summary" "selftest-summary.json")"
...
run_openclaw_agent_json "$SMOKE_JSON" \
  ...
  --to "$SELF_E164" \
  --deliver \
  --message "Nutze read, lies $SUMMARY_WORKSPACE_PATH und antworte exakt mit $EXPECTED. Gib nur diese eine Zeile aus. Kein weiterer Text. Ohne echten read-Toolcall darfst du den Auftrag nicht abschließen."

run_json_assert "$SMOKE_JSON" "$EXPECTED" >/dev/null
assert_latest_session_pattern "main" '"name":"read"'
assert_latest_session_pattern "main" "$SUMMARY_WORKSPACE_PATH"
