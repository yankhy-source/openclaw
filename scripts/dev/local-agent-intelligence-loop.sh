...
CONTEXT_FALLBACK_SMOKE_SCRIPT="$SCRIPT_DIR/local-human-whatsapp-context-fallback-smoke.sh"
CONTEXT_REPORT_AUDIT_SCRIPT="$SCRIPT_DIR/local-whatsapp-context-report-audit.sh"
...
CONTEXT_FALLBACK_SNAPSHOT_PATH=""
CONTEXT_REPORT_AUDIT_SNAPSHOT_PATH=""
CONTEXT_REPORT_AUDIT_MARKDOWN_SNAPSHOT_PATH=""
CONTEXT_REPORT_AUDIT_PATH=""
...
CONTEXT_FALLBACK_SNAPSHOT_PATH="$LOOP_ROOT/context-fallback-summary.json"
CONTEXT_REPORT_AUDIT_SNAPSHOT_PATH="$LOOP_ROOT/context-report-audit-summary.json"
CONTEXT_REPORT_AUDIT_MARKDOWN_SNAPSHOT_PATH="$LOOP_ROOT/context-report-audit.md"
...
payload = {
    ...
    "contextFallbackSmokeSummaryPath": sys.argv[14],
    "contextReportAuditSummaryPath": sys.argv[15],
    "contextReportAuditPath": sys.argv[16] or None,
    "reviewPath": sys.argv[17],
    "reviewJsonPath": sys.argv[18],
    "evaluatorAgentId": sys.argv[19] or None,
    "failedCommand": sys.argv[20] or None,
    "selftestStatus": sys.argv[21] or None,
    "selftestMode": sys.argv[22] or None,
    "humanEvalStatus": sys.argv[23] or None,
    "humanWhatsappEvalStatus": sys.argv[24] or None,
    "humanWhatsappConversationStatus": sys.argv[25] or None,
    "humanWhatsappResumeStatus": sys.argv[26] or None,
    "humanWhatsappResumeFailureStatus": sys.argv[27] or None,
    "contextFallbackSmokeStatus": sys.argv[28] or None,
    "contextReportAuditStatus": sys.argv[29] or None,
    "nextUpgrade": sys.argv[30] or None,
    "reviewText": sys.argv[31] or None,
}
...
echo "== context fallback smoke =="
bash "$CONTEXT_FALLBACK_SMOKE_SCRIPT" >/dev/null

echo "== context report audit =="
bash "$CONTEXT_REPORT_AUDIT_SCRIPT" >/dev/null
...
if [[ ! -f "$CONTEXT_FALLBACK_SMOKE_SUMMARY_PATH" ]]; then
  echo "missing context fallback smoke summary at $CONTEXT_FALLBACK_SMOKE_SUMMARY_PATH" >&2
  exit 1
fi
if [[ ! -f "$CONTEXT_REPORT_AUDIT_SUMMARY_PATH" ]]; then
  echo "missing context report audit summary at $CONTEXT_REPORT_AUDIT_SUMMARY_PATH" >&2
  exit 1
fi
...
cp "$CONTEXT_FALLBACK_SMOKE_SUMMARY_PATH" "$CONTEXT_FALLBACK_SNAPSHOT_PATH"

eval "$(python3 - <<'PY' "$SELFTEST_SUMMARY_PATH" "$HUMAN_EVAL_SUMMARY_PATH" "$HUMAN_WHATSAPP_EVAL_SUMMARY_PATH" "$HUMAN_WHATSAPP_CONVERSATION_EVAL_SUMMARY_PATH" "$HUMAN_WHATSAPP_RESUME_EVAL_SUMMARY_PATH" "$HUMAN_WHATSAPP_RESUME_FAILURE_EVAL_SUMMARY_PATH" "$CONTEXT_FALLBACK_SMOKE_SUMMARY_PATH" "$CONTEXT_REPORT_AUDIT_SUMMARY_PATH"
...
labels = [
    ...
    ("CONTEXT_FALLBACK_SMOKE_STATUS", "status"),
    ("CONTEXT_REPORT_AUDIT_STATUS", "status"),
]
...
context_fallback = json.loads(open(paths[6], "r", encoding="utf-8").read())
context_report_audit = json.loads(open(paths[7], "r", encoding="utf-8").read())
values = {
    ...
    "CONTEXT_FALLBACK_SMOKE_STATUS": context_fallback.get("status", ""),
    "CONTEXT_REPORT_AUDIT_STATUS": context_report_audit.get("status", ""),
}
for name, _ in labels:
    print(f"{name}={shlex.quote(str(values[name]))}")
print(f"CONTEXT_REPORT_AUDIT_PATH={shlex.quote(str(context_report_audit.get('auditPath') or ''))}")
PY
)"

if [[ -z "$CONTEXT_REPORT_AUDIT_PATH" || ! -f "$CONTEXT_REPORT_AUDIT_PATH" ]]; then
  echo "context report audit markdown missing at $CONTEXT_REPORT_AUDIT_PATH" >&2
  exit 1
fi
cp "$CONTEXT_REPORT_AUDIT_SUMMARY_PATH" "$CONTEXT_REPORT_AUDIT_SNAPSHOT_PATH"
cp "$CONTEXT_REPORT_AUDIT_PATH" "$CONTEXT_REPORT_AUDIT_MARKDOWN_SNAPSHOT_PATH"
...
if [[ "$CONTEXT_FALLBACK_SMOKE_STATUS" != "passed" ]]; then
  echo "context fallback smoke did not pass: $CONTEXT_FALLBACK_SMOKE_STATUS" >&2
  exit 1
fi
if [[ "$CONTEXT_REPORT_AUDIT_STATUS" != "passed" ]]; then
  echo "context report audit did not pass: $CONTEXT_REPORT_AUDIT_STATUS" >&2
  exit 1
fi
...
wait_for_session_pattern_after_line "$EVALUATOR_SESSION" "$EVALUATOR_BEFORE_LINES" "$CONTEXT_FALLBACK_SNAPSHOT_PATH" 80 1
wait_for_session_pattern_after_line "$EVALUATOR_SESSION" "$EVALUATOR_BEFORE_LINES" "$CONTEXT_REPORT_AUDIT_SNAPSHOT_PATH" 80 1
wait_for_session_pattern_after_line "$EVALUATOR_SESSION" "$EVALUATOR_BEFORE_LINES" "$CONTEXT_REPORT_AUDIT_MARKDOWN_SNAPSHOT_PATH" 80 1
...
printf 'context_fallback_smoke_summary=%s\n' "$CONTEXT_FALLBACK_SMOKE_SUMMARY_PATH"
printf 'context_report_audit_summary=%s\n' "$CONTEXT_REPORT_AUDIT_SUMMARY_PATH"
printf 'context_report_audit=%s\n' "$CONTEXT_REPORT_AUDIT_PATH"
